...
As of 2026-07-09, the implemented milestone is relay-backed pairing plus browser FIPS discovery:
...
- After approval, WebVM starts a browser `@fips/core` node over `@fips/transport-webrtc` using the same AppKey secret and publishes a kind `37195` `fips-overlay-v1` advert.
- The Playwright e2e starts a local Nostr relay, subscribes from WebVM, publishes a native-shaped approval receipt, verifies that the VM auto-detects the paired state, and verifies that the paired AppKey publishes the FIPS/WebRTC advert.
...
4. Replace the vendored local `@fips/core` and `@fips/transport-webrtc` tarballs with a released package source after those `0.0.1` packages are published from the shared FIPS TypeScript library.
5. Connect virtual NIC packets to the browser FIPS endpoint-data API (`sendEndpointData` / `endpointData`) and route them through native Nostr VPN exit nodes. This fork includes a tested endpoint-data bridge boundary and a browser FIPS session boundary, but the boundary is not wired to a running packet-capable VM backend yet.
