...
test('real WebVM guest auto-pairs through NativeAppAction and reaches HTTPS over nvpn0', async ({ page }) => {
	...
	try {
		...
		const gitRemoteSmoke = await runSerialCommand(
			...
			"repo=/run/webvm/e2e-git; clone=/run/webvm/e2e-git-clone; rm -rf \"$repo\" \"$clone\"; mkdir -p \"$repo\"; git -C \"$repo\" init -b main >/dev/null; git -C \"$repo\" config user.name 'WebVM E2E'; git -C \"$repo\" config user.email webvm-e2e@iris.localhost; printf 'git-remote-htree-e2e' >\"$repo/proof.txt\"; git -C \"$repo\" add proof.txt; git -C \"$repo\" commit -m initial >/dev/null; git -C \"$repo\" remote add htree htree://self/webvm-e2e; helper_env='HTREE_LOCAL_DAEMON_ONLY=1 HTREE_CONFIG_DIR=/var/lib/hashtree/config HTREE_DATA_DIR=/var/lib/hashtree/data'; push_output=$(env $helper_env git -C \"$repo\" push --force htree main 2>&1); push_rc=$?; published_url=$(printf '%s\\n' \"$push_output\" | sed -n 's/^Published to: //p' | tail -1); published_npub=$(printf '%s\\n' \"$published_url\" | cut -d/ -f3); resolve_output=$(curl --noproxy '*' -fsS \"http://127.0.0.1:80/api/nostr/resolve/${published_npub}/webvm-e2e\" 2>&1) || resolve_output=failed; clone_output=; clone_rc=1; if [ $push_rc -eq 0 ] && [ -n \"$published_url\" ]; then clone_output=$(env $helper_env git clone \"$published_url\" \"$clone\" 2>&1); clone_rc=$?; fi; if [ $push_rc -eq 0 ] && [ $clone_rc -eq 0 ] && [ \"$(cat \"$clone/proof.txt\" 2>/dev/null)\" = git-remote-htree-e2e ]; then printf 'GIT_REMOTE_HTREE_OK\\n'; else printf 'GIT_REMOTE_HTREE_FAILED:push=%s:clone=%s:url=%s:resolve=%s\\n' \"$push_rc\" \"$clone_rc\" \"$published_url\" \"$resolve_output\"; printf '%s\\n%s\\n' \"$push_output\" \"$clone_output\" | tail -20; printf '%s\\n' '--- HASHTREE LOG ---'; tail -30 /var/log/webvm-hashtree.log 2>/dev/null; fi; rm -rf \"$repo\" \"$clone\"",
			120_000,
		);
